/**
 * Game Summary Page
 * Shows game-level aggregated report data with advanced filtering and export capabilities
...
import {
  ...
  getRTPColorClass,
  getGGRColorClass,
  gameSummaryExportFields,
} from '@/lib/utils/summary'
import type { GameSummaryData, GameSummaryFilter } from '@/lib/types/summary'
...
export function GameSummaryPage() {
  ...
  const [filters, setFilters] = useState<GameSummaryFilter>({
  ...
  const summaryQuery = useGameSummary(queryFilters)
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Game',
      sortable: true,
      render: (value: string, row: GameSummaryData) => (
        <div>
          <div className='font-semibold text-gray-950 dark:text-white'>{value}</div>
          <div className='text-sm text-gray-600 dark:text-gray-300 font-medium'>{row.platform}</div>
        </div>
      ),
    },
    {
      ...
      sortable: true,
      render: (value: number) => (
        <span className='font-mono text-gray-950 dark:text-white font-semibold text-base'>
          {formatNumber(value)}
        </span>
      ),
    },
    {
      ...
      label: 'Won Rounds',
      sortable: true,
      render: (value: number, row: GameSummaryData) => (
        <div className='flex items-center gap-1'>
          <span className='font-mono text-green-600 font-semibold'>{formatNumber(value)}</span>
          <span className='text-xs text-gray-500'>
            ({((value / row.totalRounds) * 100).toFixed(1)}%)
          </span>
        </div>
      ),
    },
  ...
  const handleFiltersChange = (newFilters: Partial<GameSummaryFilter>) => {
    ...
    <div className='w-full max-w-full overflow-x-hidden'>
      <div className='py-4 sm:py-6 space-y-4 sm:space-y-6 px-3 sm:px-4'>
        ...
        <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
          <div>
            <h1 className='text-3xl font-bold'>Game Summary</h1>
            <p className='text-gray-600 mt-2'>
              View game-level aggregated gaming data with comprehensive analytics and insights.
            </p>
          </div>
        ...
        <SummaryTable
          ...
          onSort={handleSort}
          exportFields={gameSummaryExportFields}
          exportFilename='game_summary'
          searchPlaceholder='Search by game, platform, amounts...'
          showSummary={true}
        />
      </div>
    </div>
