/**
 * Player Game Summary Page
 * Shows player-game level aggregated report data with advanced filtering and export capabilities
...
import { usePlayerGameSummary, useSummaryData } from '@/hooks/queries/useSummaryQueries'
...
import {
  ...
  getRTPColorClass,
  getGGRColorClass,
  playerGameSummaryExportFields,
} from '@/lib/utils/summary'
import type { PlayerGameSummaryData, PlayerGameSummaryFilter } from '@/lib/types/summary'
...
export function PlayerGameSummaryPage() {
  ...
  const [filters, setFilters] = useState<PlayerGameSummaryFilter>({
  ...
  const summaryQuery = usePlayerGameSummary(queryFilters)
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Won Rounds',
      sortable: true,
      render: (value: number, row: PlayerGameSummaryData) => (
  ...
  const handleFiltersChange = (newFilters: Partial<PlayerGameSummaryFilter>) => {
    ...
    <div className='w-full max-w-full overflow-x-hidden'>
      <div className='py-4 sm:py-6 space-y-4 sm:space-y-6 px-3 sm:px-4'>
        ...
        <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
          <div>
            <h1 className='text-3xl font-bold'>Player Game Summary</h1>
            <p className='text-gray-600 mt-2'>
              View player-game level aggregated gaming data with comprehensive analytics and
              insights.
            </p>
          </div>
        ...
        <SummaryTable
          ...
          onSort={handleSort}
          exportFields={playerGameSummaryExportFields}
          exportFilename='player_game_summary'
          searchPlaceholder='Search by player, game, rounds, amounts...'
          showSummary={true}
        />
      </div>
    </div>
